...
struts = []
strut = 0.0
max_strut_thickness = 50.0 #microns
while strut <= max_strut_thickness:
    struts.append(strut)
    strut += 0.5
optimization = []
for strut_thickness in struts:
    ...
    while i < N:
        ...
        neutron = Path()
        neutron.strut_thick_mean = strut_thickness*1E-4
        neutron.strut_thick_sigma = 0.25*neutron.strut_thick_mean
        ...
        if x > neutron.maximum_length:
            ...
            alpha = Path()
            alpha.strut_thick_mean = strut_thickness*1E-4
            alpha.strut_thick_sigma = 0.25*alpha.strut_thick_mean
            triton = Path()    
            triton.strut_thick_mean = strut_thickness*1E-4
            triton.strut_thick_sigma = 0.25*triton.strut_thick_mean
    ...
    optimization.append((strut_thickness, detection_efficiency, math.sqrt(detected)/N*100))
# ==================================================================================
...
plt.xlabel('Average Foam Strut Thickness (um)')
plt.ylabel('Detection Efficiency (%)')
plt.xlim(0, max_strut_thickness)
plt.ylim(0, 100)
plt.errorbar([l[0] for l in optimization], [l[1] for l in optimization], [l[2] for l in optimization], fmt='o', color='k')
np.savetxt("optimzation_results.csv", optimization, delimiter=',')